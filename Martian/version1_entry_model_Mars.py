...
import os
import sys
from mars_atmospheric_cal import atmospheric_density_and_oxygen
from contextlib import redirect_stdout

MARS_RAD = 3.39E6 # radius of Mars
MARS_MASS = 6.39E23 #Mars mass [kg]
GRAV_CONST = 6.67E-11 #gravitational constant [N m2 kg-2]
GRAV_CONST_MARS = GRAV_CONST #gravitational constant [N m2 kg-2] still for mars
...
class initialVelocityDistribution(stats.rv_continuous):
    ...
    def __init__(self):
        ...
        """
        super().__init__(a=10, b=20.0) #upper limit set to 20 km s-1
...
class initialMassDistribution(stats.rv_continuous):
    ...
    def __init__(self):
        ...
        """
        super().__init__(a=2E-10, b=2.35e-2)
...
def simulate_particle(input_mass, input_vel, input_theta):
    """
    ...
    def should_end(_, y_cur, tracker):
        ...
        later). We also stop if the particle is moving away from the MARS due 
    ...
    def sim_func(time, y_in, tracker):
        ...
        #calculate gravity
        gravity = GRAV_CONST*MARS_MASS/alt**2
        ...
        rho_a, rho_co2, rho_o , rho_o2 = atmospheric_density_and_oxygen(alt)
        #if co2_percent != -1:
        ...
        add_co2 = 0
        ram_p =  1/2*vel**2*rho_co2 #[kg m-1 s-2]
        #A = 0
        if temp > temp_thre:
            ...
            S = pi*rad**2 #m2
            add_o1 = vel*S*rho_o #[kg s-1] 
            add_o2 = vel*S*rho_o2 #[kg s-1]
            add_o = add_o1 + add_o2 * 2
            k_rate = fe_co2_rate_pressure(temp, ram_p) #mol m-2 s-1 Pa-1
            add_co2 = vel*M_O*S*rho_co2/M_CO2 #[kg s-1]
        ...
        dq_ox_dt = DELTA_H_OX*(M_FEO/M_O)*ox_enc
        ...
        #print(f"{vel:.6g}, {temp:.6g},{(alt-MARS_RAD)/1000:.6g}, {ox_enc:.3g}, {rate_co2:.3g}, {add_o1: .3g}, {add_co2:.3g}, {ram_p:.6g},{mass_fe:.5g},{mass_feo:.5g},{add_o2:.3g}")
        
        #print(f"{vel:.6g}, {temp:.6g}, {(alt-MARS_RAD)/1000:.6g}, {ox_enc:.3g}, {rate_co2:.3g}, {add_o: .3g}, {add_co2:.3g}, {ram_p:.6g},{mass_fe:.5g},{mass_feo:.5g}")
    ...
    y_0 = [200000+MARS_RAD, #initial altitude, 200 [km]
...
def multithreadWrapper(args):
    ...
    try:
        ...
        if status < 1:
            ...
            print("mass: %2.2e, vel: %0.1f [km s-1], theta: %0.1f"%(
                mass, velocity/1000, theta*180/pi
                ))
            print("------------------------------------------------")
    except:
...
def plot_particle_parameters(input_mass, input_vel, input_theta,
    ...
    print('::This is running for modern Mars Atmosphere::')
    ...
    print("Altitude of max temp: %0.1f [km]"%((alts[ind]-MARS_RAD)/1000))
    ...
    ind = np.argmax(temps)
    max_temp = temps[ind]
    altitude = (alts[ind]-MARS_RAD)/1000
...
if __name__ =='__main__':
    ...
    if batch:
        ...
        generateRandomSampleData(num_samples=500, output_dir=output_dir, input_dir="") #"./Melt_Scenario2"

    else:
        res = plot_particle_parameters(10.665E-9, 7000, 45*pi/180)