...
def atmospheric_density_and_oxygen(altitude, co2_surface_vol, co2_lower, co2_upper):
    ...
    ratio = round((co2_upper-co2_surface_vol)/(co2_upper-co2_lower),2) #get the ratio that for lower ones directly
    #print(co2_upper, co2_lower, ratio)
    # read the file
    ...
    # 分别插值
    o_atomic = co2_lower_file['O']  * ratio + co2_upper_file['O']  * (1 - ratio)
    o2_mol   = co2_lower_file['O2'] * ratio + co2_upper_file['O2'] * (1 - ratio)

    # 用于氧化反应的"总氧当量"（氧原子数）
    o_value = o_atomic + 2 * o2_mol

    n2_mol = 1 - co2_value - o_atomic - o2_mol
    denom = co2_value*44 + o_atomic*16 + o2_mol*32 + n2_mol*28

    co2_value_wt = co2_value * 44 / denom
    o_value_wt   = (o_atomic*16 + o2_mol*32) / denom 
    ...
    if 100 <= alt < 190:
        ...
        max_idx = len(o_value_wt) - 1  
        if idx < max_idx:

            o_wt   = float(o_value_wt.iloc[idx])   * frac_low + float(o_value_wt.iloc[idx+1])   * (1 - frac_low)
            co2_wt = float(co2_value_wt.iloc[idx]) * frac_low + float(co2_value_wt.iloc[idx+1]) * (1 - frac_low)
        else:
        
            o_wt   = float(o_value_wt.iloc[idx])
            co2_wt = float(co2_value_wt.iloc[idx])
